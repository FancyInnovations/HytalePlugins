...
public class ScoreboardPageSetHeightCMD extends AbstractPlayerCommand {
    ...
    private final RequiredArg<Integer> heightArg = withRequiredArg("height", "New height", ArgTypes.INTEGER);

    public ScoreboardPageSetHeightCMD() {
        super("setheight", "Set the height of a scoreboard page");
        requirePermission("fancycore.commands.scoreboard.page.setheight");
    }
    ...
    protected void execute(@NotNull CommandContext ctx, @NotNull Store<EntityStore> store, @NotNull Ref<EntityStore> ref, @NotNull PlayerRef playerRef, @NotNull World world) {
        ...
        int height = heightArg.get(ctx);
        if (height <= 0) {
            ctx.sendMessage(Message.raw("Invalid height. Please specify a positive integer."));
            return;
        }
        ...
        fp.sendMessage("Scoreboard page '" + page.getName() + "' height set to " + height + ".");
    }
}
